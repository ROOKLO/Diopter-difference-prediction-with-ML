...
nn_space = {'nn':
                {'layer0':hp.choice('layer0', np.arange(32, 1024, 32)),
                'layer1':hp.choice('layer1', np.arange(32, 1024, 64)),
                'layer2':hp.choice('layer2', np.arange(32, 1024, 64)),
                'layer3':hp.choice('layer3', np.arange(32, 1024, 64)),
                'layer4':hp.choice('layer4', np.arange(32, 1024, 64)),
                # 'layer5':hp.choice('layer5', np.arange(64, 2049, 64)),
                'dropout':hp.uniform('dropout', 0, 0.5)},

            'batch_size': hp.choice('batch_size', np.arange(128, 1024, 64)),
            'learning_rate': hp.uniform('learning_rate', 1e-05, 5e-03),
            'epoch': 300
            }
...
class MLP_net(torch.nn.Module):
    def __init__(self, num_inputs, num_outputs, **param):
        ...
        self.hidden_1 = torch.nn.Linear(param['layer0'], param['layer1'])
        ...
        self.hidden_3 = torch.nn.Linear(param['layer2'], param['layer3'])
        self.bn3 = torch.nn.BatchNorm1d(param['layer3'])

        self.hidden_4 = torch.nn.Linear(param['layer3'], param['layer4'])
        self.bn4 = torch.nn.BatchNorm1d(param['layer4'])

        # self.hidden_5 = torch.nn.Linear(param['layer4'], param['layer5'])
        # self.bn5 = torch.nn.BatchNorm1d(param['layer5'])

        self.out = torch.nn.Linear(param['layer4'], out_features=num_outputs)
...
def train_mlp(x_data, y_data, cv_num, **param):
    ...
    if label == 'label0.25':
        posi_weight = torch.tensor(0.5)
    elif label == 'label0.75':
        posi_weight = torch.tensor(2.3)
    else:
        posi_weight = torch.tensor(1)
    ...
    for i, index in enumerate(kf.split(x_data)):
        ...
        for epoch in range(param['epoch']):
            ...
            for x, y in train_loader:
                ...
                # loss = criterion(pred, y)
                # # print(f'Loss({i}-{epoch})', loss.item())
        ...
        # pbar1.update(1)
            ...
            with torch.no_grad():
                ...
                acc_balanced = metrics.balanced_accuracy_score(y_true=y_test, y_pred=test_pre_label)

            acclist.append(acc_balanced)

        accmax_list.append(np.max(acclist))
    acc_mean = np.mean(accmax_list)
...
if __name__ == '__main__':
    ...
    datas = dict({'p1': train_set_p1})

    for p in datas:
        x = datas[p].iloc[:, :35]
        for label in label_binary:
            print(f'{p}-{label}-mlp'.center(40, '*'))  # *********
            y = datas[p][label]

            def object_function(param):
                acc = train_mlp(x_data=x, y_data=y, cv_num=5, **param)
